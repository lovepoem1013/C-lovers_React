...
const NaviModal = () => {
  ...
  const [officeManager, setOfficeManager] = useState(false);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < authority.length; i++) {
      ...
      if (authority[i] === "회계" || authority[i] === "총괄") {
        setAccountingManager(true);
      }
      if (authority[i] === "총괄") {
        setOfficeManager(true);
      }
    }
  }, [authority]);
  useEffect(() => {
    console.log(officeManager);
  }, [officeManager]);
  useEffect(() => {
    console.log(accountingManager);
  }, [accountingManager]);
